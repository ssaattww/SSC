...
- Status: In Progress
- Notes:
  - T-094でPR #49の独立最終レビュー指摘 `PR49-FR1` を受け、空文字列keyのlegacy標準pathにも祖先patternが一致する修正をTDDで進行中
...
- Status: In Progress
- Notes:
  - T-094はterra/high実装後にfocused/full test、format、diff check、matching-HEAD CI、fix verification、fresh独立最終レビューを実施する
